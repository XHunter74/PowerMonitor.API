...
import { MqttDataModel } from '../models/mqtt-data.model';
...
export class MqttClientService {
    ...
    constructor(
        ...
        this.mqttConnectionService.onConnect(() => {
            this.logger.debug(`[${MqttClientService.name}].Constructor => MQTT client is connected`);
        });
        this.mqttConnectionService.onError((error) => {
            this.logger.error(`[${MqttClientService.name}].Constructor => MQTT client error: '${JSON.stringify(error)}'`);
        });
        this.clientSubscription = this.collectDataService.getSensorsData.subscribe(async (data: SensorsData) => {
    ...
    public async publishCurrentAndVoltageData(sensorsData: SensorsData) {
        if (!this.config.IsDevEnvironment && this.mqttConnectionService.isConnected()) {
            ...
            const data = {
                ...
                Power: Math.round(sensorsData.voltage * sensorsData.amperage / 1000 * 100) / 100,
            };
            this.mqttConnectionService.publish('tele/tasmota/STATE', JSON.stringify(data), options);
        }
    }
    ...
    private async getMqttData(): Promise<MqttDataModel> {
        ...
        const todayPowerData = await this.powerDataService.getPowerDataHourly(today, today, { suppressLogging: true });
        let power = todayPowerData.reduce((a, b) => a + b.power, 0);
        power = Math.round(power * 100) / 100;
        result.Today = power;
        const startDate = new Date(today.getFullYear(), today.getMonth(), 1);
        const endDate = new Date(today.getFullYear(), today.getMonth(), daysInMonth(today.getFullYear(), today.getMonth() + 1));
        const dataForMonth = await this.powerDataService.getPowerDataDaily(startDate, endDate, { suppressLogging: true });
        ...
        result.Uptime = await this.servicesService.getSystemUptimeSeconds();
        result.SerialState = this.collectDataService.SerialPortState ? 1 : 0;
        return result;
    }

    async processApplicationShutdown(signal?: string) {
        this.logger.info(`[${MqttClientService.name}].${this.processApplicationShutdown.name} => Start`);
        this.logger.info(`[${MqttClientService.name}].${this.processApplicationShutdown.name} => ` +
            `Received shutdown signal: '${signal}'`);
        this.clientSubscription.unsubscribe();
        this.clientSubscription = null;
        this.logger.info(`[${MqttClientService.name}].${this.processApplicationShutdown.name} => Finish`);
    }
}
