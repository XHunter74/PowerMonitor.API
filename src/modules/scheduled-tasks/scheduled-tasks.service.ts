...
export class ScheduledTasksService implements OnApplicationShutdown {
    ...
    async runPeriodically() {
        try {
            ...
            this.logger.debug(
                `[${ScheduledTasksService.name}].updatePowerAvailability => ` + `Error : ${error}`,
            );
        }
        try {
            ...
            this.logger.debug(
                `[${ScheduledTasksService.name}].checkSerialAvailability => ` + `Error : ${error}`,
            );
        }
        try {
            ...
            this.logger.debug(
                `[${ScheduledTasksService.name}].publishPowerData => ` + `Error : ${error}`,
            );
        }
    }
    ...
    async runHourly() {
        ...
        try {
            ...
            this.logger.error(
                err.message,
                `[${ScheduledTasksService.name}].${this.runHourly.name} => ` +
                    `Exception occurred ^^^`,
            );
        }
    ...
    private async deleteOutdatedVoltageData() {
        this.logger.info(
            `[${ScheduledTasksService.name}].${this.deleteOutdatedVoltageData.name} => ` + `Start`,
        );
        const now = new Date();
        ...
        this.logger.info(
            `[${ScheduledTasksService.name}].${this.deleteOutdatedVoltageData.name} => ` + `Finish`,
        );
    }

    private async deleteOutdatedTokens() {
        this.logger.info(
            `[${ScheduledTasksService.name}].${this.deleteOutdatedTokens.name} => Start`,
        );
        ...
        const tokensForDelete = await this.tokensRepository
            .createQueryBuilder('token')
            .where('token.created < :expirationDate', { expirationDate })
            .getMany();

        this.logger.info(
            `[${ScheduledTasksService.name}].${this.deleteOutdatedTokens.name} => ` +
                `Will be deleted '${tokensForDelete.length}' tokens`,
        );
        ...
        this.logger.info(
            `[${ScheduledTasksService.name}].${this.deleteOutdatedTokens.name} => Finish`,
        );
    }

    onApplicationShutdown(signal?: string) {
        this.logger.info(
            `[${ScheduledTasksService.name}].${this.onApplicationShutdown.name} => Start`,
        );
        this.logger.info(
            `[${ScheduledTasksService.name}].${this.onApplicationShutdown.name} => ` +
        ...
        this.logger.info(
            `[${ScheduledTasksService.name}].${this.onApplicationShutdown.name} => Finish`,
        );
    }
}
