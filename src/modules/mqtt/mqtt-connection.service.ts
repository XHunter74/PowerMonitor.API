...
export class MqttConnectionService {
    ...
    constructor(
        ...
        this.client.on('error', (error) => {
            this.logger.error(
                `[${MqttConnectionService.name}] => MQTT connection error: ${error.message}`,
            );
        });
    ...
    public publish(topic: string, message: string, options?: Record<string, any>): void {
        if (this.isConnected()) {
            this.client.publish(topic, message, options, (error) => {
                if (error) {
                    this.logger.error(
                        `[${MqttConnectionService.name}] => Publish error: ${error.message}`,
                    );
                }
            });
        } else {
            this.logger.error(
                `[${MqttConnectionService.name}] => Cannot publish, MQTT client is not connected`,
            );
        }
    }
